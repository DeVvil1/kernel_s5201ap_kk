...
#define MTKFB_LOG_D(fmt, arg...) \
	do { \
		if (mtkfb_log_on) DISP_LOG_PRINT(ANDROID_LOG_DEBUG, "MTKFB", fmt, ##arg); \
	}while (0)
...
atomic_t fb_open_cnt = ATOMIC_INIT(0);
...
#if defined(__EKTF2K_FT__)  // Added by 2014/09/24
extern void LCD_WaitTE(void);
#endif
...
void mtkfb_fb_change_active_page(unsigned int page)
{
    if (!DISP_IsDecoupleMode())
    {
        unsigned int pa, va, size, fb_va;
        ...
        size = mtkfb_fbi->var.yres * mtkfb_fbi->fix.line_length;
        pa = fb_pa + size * page;
        if ((realtime_layer_config[FB_LAYER].addr != pa) && (realtime_layer_config[FB_LAYER].addr != 0))
        {
            fb_va = (unsigned int)((struct mtkfb_device *)mtkfb_fbi->par)->fb_va_base;
            va = fb_va + size * page;
            printk("mtkfb_fb_change_active_page %d : copy frame from %X to %X", page, realtime_layer_config[FB_LAYER].addr, pa);
            memcpy((void *)va, (void *)realtime_layer_config[FB_LAYER].addr - fb_pa + fb_va, size);
            cached_layer_config[FB_LAYER].vaddr = va;
            cached_layer_config[FB_LAYER].addr = pa;
            ...
            wait_event_timeout(reg_update_wq, PanDispSettingApplied, HZ/10);
        }
...
#endif
{
    ...
    atomic_dec(&fb_open_cnt);

    if ((atomic_read(&fb_open_cnt) == 0) &&
        ((lcm_params->type == LCM_TYPE_DPI) ||
         ((lcm_params->type == LCM_TYPE_DSI) && (lcm_params->dsi.mode != CMD_MODE))))
        mtkfb_fb_change_active_page(MTK_FB_PAGES - 1);
...
static void mtkfb_fb_565_to_888(void* fb_va)
{
    ...
    printk("555_to_888, s = 0x%p, d=0x%p\n", s,d);
    for (j = 0; j < yres; ++ j)
    {
        for(k = 0; k < xres; ++ k)
        {
