...
/**<begin>add music key in long press mode	.add by neal*/
//#define MUSIC_KEY_SWITCH		1
...
/**<begin>add music key in long press mode	.add by neal*/
#if MUSIC_KEY_SWITCH
#define MUSIC_DELAY_TIME_5S	300
static struct hrtimer music_timer_5s;
static bool music_timer_5s_started;
static bool timer_finish = false;
u16 tem_keycode;
bool tem_pressed = false;
#endif

/**<end>add music key in long press mode	.add by neal*/
...
/**<begin>add music key in long press mode	.add by neal*/
#if MUSIC_KEY_SWITCH
static enum hrtimer_restart music_timer_5s_func(struct hrtimer *timer) {
	
	printk("kpd: music_timer_5s_func !\n");
	if(tem_pressed)
	{	
		printk("kpd: music_timer_5s_func----1 !\n");
		if(tem_keycode == 163||tem_keycode == 100)
		{
			input_report_key(kpd_input_dev, KEY_FORWARD, 1);
			input_sync(kpd_input_dev);
		}
		else if(tem_keycode == 165||tem_keycode == 42)
		{
			input_report_key(kpd_input_dev, KEY_REWIND, 1);
			input_sync(kpd_input_dev);
		}
		else if(tem_keycode == 164)
		{
			input_report_key(kpd_input_dev, KEY_STOP, 1);
			input_sync(kpd_input_dev);
		}
	}
	else
	{
		printk("kpd: music_timer_5s_func----2 !\n");
		input_report_key(kpd_input_dev, tem_keycode, 1);
		input_report_key(kpd_input_dev, tem_keycode, 0);
		input_sync(kpd_input_dev);
	}
	//if(hrtimer_cancel(&music_timer_5s))
	{
		//kpd_print("try to cancel music hrtimer (5s) \n");
	}
	timer_finish = true;
	return HRTIMER_NORESTART;
}
...
static void kpd_keymap_handler(unsigned long data)
{
	...
	for (i = 0; i < KPD_NUM_MEMS; i++) {
		...
		for (j = 0; j < 16; j++) {
			...
			/**<begin>add music key in long press mode	.add by neal*/
			#if MUSIC_KEY_SWITCH
			if(linux_keycode==163 || linux_keycode==165 || linux_keycode==164 ||linux_keycode==100||linux_keycode==42)
			{		
				tem_keycode = linux_keycode;
				tem_pressed = pressed;
				if(tem_pressed)
				{
				timer_finish = false;
				hrtimer_start(&music_timer_5s, 
						ktime_set(MUSIC_DELAY_TIME_5S/1000,(MUSIC_DELAY_TIME_5S % 1000) * 1000000),
						HRTIMER_MODE_REL);
				}
				if(timer_finish)
				{
					if(tem_keycode == 163||tem_keycode == 100)
					{
						input_report_key(kpd_input_dev, KEY_FORWARD, 0);
						input_sync(kpd_input_dev);
					}
					else if(tem_keycode == 165||tem_keycode == 42)
					{
						input_report_key(kpd_input_dev, KEY_REWIND, 0);
						input_sync(kpd_input_dev);
					}
					else if(tem_keycode == 164)
					{
						input_report_key(kpd_input_dev, KEY_STOP, 0);
						input_sync(kpd_input_dev);
					}
				}
				//input_report_key(kpd_input_dev, KEY_FORWARD, pressed);
				//input_sync(kpd_input_dev);
			}
			else
			{
...
/**<begin>add music key in long press mode	.add by neal*/
#if MUSIC_KEY_SWITCH
	__set_bit(KEY_FORWARD, kpd_input_dev->keybit);
	__set_bit(KEY_REWIND, kpd_input_dev->keybit);
	__set_bit(KEY_STOP, kpd_input_dev->keybit);
#endif
/**<end>add music key in long press mode	.add by neal*/
...
/**<begin>add music key in long press mode	.add by neal*/
#if MUSIC_KEY_SWITCH
	hrtimer_init(&music_timer_5s, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	music_timer_5s.function = music_timer_5s_func;
#endif
/**<end>add music key in long press mode	.add by neal*/
