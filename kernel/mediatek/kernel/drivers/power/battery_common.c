...
static struct workqueue_struct * battery_workqueue = NULL;
...
static DEFINE_MUTEX(bat_update_event_mutex);
...
#else
	{
	    mutex_lock(&bat_update_event_mutex);
		wireless_update(&wireless_main);
		battery_update(&battery_main);			
		ac_update(&ac_main);
		usb_update(&usb_main);
        mutex_unlock(&bat_update_event_mutex);
	}
...
CHARGER_TYPE mt_charger_type_detection(void)
{
    ...
    if(BMT_status.charger_type == CHARGER_UNKNOWN)
    {
        ...
        #if defined(MTK_KERNEL_POWER_OFF_CHARGING) && defined(MTK_PUMP_EXPRESS_SUPPORT)
 	    if (BMT_status.UI_SOC == 100)
		{
...
static void mt_kpoc_power_off_check(void)
{
	...
	{
		if( (upmu_is_chr_det() == KAL_FALSE)  && (charger_vol < 2500))	//vbus < 2.5V
		{
...
void do_chrdet_int_task(void)
{
    ...
    if(g_bat_init_flag == KAL_TRUE)
    {
...
#ifdef S4043BP_FACTORY_FIX_AC_CHARGITNG_CURRENT
        ...
        #endif
        {   
            ret = queue_work(battery_workqueue, &battery_work);		    
            if(!ret)	    
            {            
                battery_xlog_printk(BAT_LOG_CRTI,"[Battery]battery_work return:%d!\n", ret);               
            }
...
{  
    ...
    queue_work(battery_workqueue, &battery_work);
}
...
int bat_thread_kthread(void *x)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "bat_thread_kthread init done\n" );
...
static void battery_work_callback(struct work_struct *work)
{   

    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] battery_work_callback, cable in/out.\n");      
    mt_battery_charger_detect_check();  
    if (BMT_status.UI_SOC == 100 && BMT_status.charger_exist == KAL_TRUE && (BMT_status.bat_in_recharging_state == KAL_FALSE))   
    {       
        BMT_status.bat_charging_state = CHR_BATFULL;        
        BMT_status.bat_full = KAL_TRUE;     
        g_charging_full_reset_bat_meter = KAL_TRUE; 
    ...
    //battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] workqueue test.\n"); 
    //mdelay(5000); if(g_battery_soc_ready == KAL_FALSE)    
    ...
    mt_battery_update_status(); 
    battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] battery_work_callback, update uevent.\n");
}
...
static int battery_probe(struct platform_device *dev)    
{
    ...
    INIT_WORK(&battery_work,battery_work_callback);
