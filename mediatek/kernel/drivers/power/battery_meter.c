...
#else
#define TemperatureR_MAX	300000 //Line define max TemperatureR added by zx, it's used when not battery power
int BattThermistorConverTemp(int Res)
{
    int i=0;
    int RES1=0,RES2=0;
    ...
    bm_print(BM_LOG_CRTI, "[BattThermistorConverTemp] Res:%d, TemperatureR_MAX:%d\n", Res,TemperatureR_MAX);
    if(Res > TemperatureR_MAX)
    {
		TBatt_Value = 25;	//when not battery power,it's as normal temperature
    }
...
void dod_init(void)
{
    ...
    if(((g_rtc_fg_soc != 0) && (((abs(g_rtc_fg_soc-gFG_capacity_by_v)) < CUST_POWERON_DELTA_CAPACITY_TOLRANCE)||(abs(g_rtc_fg_soc-g_booting_vbat)< CUST_POWERON_DELTA_CAPACITY_TOLRANCE)) &&(( gFG_capacity_by_v > CUST_POWERON_LOW_CAPACITY_TOLRANCE || bat_is_charger_exist() == KAL_TRUE)))
