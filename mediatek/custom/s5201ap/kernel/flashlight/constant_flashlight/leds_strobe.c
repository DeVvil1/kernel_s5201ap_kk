...
static int capture_flash_flag = 0; //for capture flash open
...
#define LEDS_LOWHIGH_END_FLAG  0  // 0 low-end led    1 high-end led    2 perfect-end
...
int FL_GetCaputureFLStatus(void){
   PK_DBG("FL_GetCaputureFLStatus, %d\n", capture_flash_flag);
   return capture_flash_flag;
}
EXPORT_SYMBOL(FL_GetCaputureFLStatus);

void FL_SetCaputureFLStatus(int flag){
   PK_DBG("FL_SetCaputureFLStatus, %d\n", flag);
   capture_flash_flag = flag;
}
EXPORT_SYMBOL(FL_SetCaputureFLStatus);
