...
int ltr559_init_device(struct i2c_client *client)
{
    ...
    hwmsen_write_byte(client, 0x83, 0x0c); //0x06              ///5 pulse
...
static int ltr553_ps_set_thres()
...
static void ltr559_eint_work(struct work_struct *work)
{
    ...
    if ((1 << CMC_BIT_PS) & obj->pending_intr)
    {
        ...
        if(sensor_data.values[0] == 0)
        {
            ...
            hwmsen_write_byte(obj->client,0x92,(u8)(hw->ps_threshold_low) & 0xff);              //yaoyaoqin:low threshold for faraway interrupt only
            hwmsen_write_byte(obj->client,0x93,(u8)((hw->ps_threshold_low)>>8) & 0X07);
...
#endif
	APS_ERR("ltr599_eint_work High: %d,Low: %d\n", hw->ps_threshold_high,hw->ps_threshold_low);
            //wake_lock_timeout(&ps_wake_lock,ps_wakeup_timeout*HZ);
            hwmsen_write_byte(obj->client,0x90,(u8)(hw->ps_threshold_high) & 0xff);             //yaoyaoqin:high threshold for close interrupt only
            hwmsen_write_byte(obj->client,0x91,(u8)((hw->ps_threshold_high)>>8) & 0X07);
...
APS_ERR("ltr559_dynamic_calibrate High:%d,Low:%d\n",hw->ps_threshold_high,hw->ps_threshold_low);	
    hwmsen_write_byte(ltr559_obj->client, 0x90, (u8)(hw->ps_threshold_high) & 0XFF);
    hwmsen_write_byte(ltr559_obj->client, 0x91, (u8)((hw->ps_threshold_high)>>8) & 0X07);
    hwmsen_write_byte(ltr559_obj->client, 0x92, (u8)(hw->ps_threshold_low) & 0xff);
    hwmsen_write_byte(ltr559_obj->client, 0x93, (u8)((hw->ps_threshold_low)>>8) & 0x07);
...
static void ap3220_ps_cali_set_threshold(void)
{
    ...
    if( value_high > 0x32)
    {
        value_high= 0x32;
        value_low= 0x23;
        APS_LOG("ap3220_ps_cali_set_threshold: set value_high=0x32,value_low=0x23, please check the phone \n");
    }

    atomic_set(&ltr559_obj->ps_thd_val_high, value_high);
    atomic_set(&ltr559_obj->ps_thd_val_low,  value_low);
...
static void ap3220_ps_cali_start(void)
{
    ...
    if(NULL == ltr559_obj->client)
    {
        APS_ERR("ltr559_obj->client == NULL\n");
        return;
    }
    ...
    */
    ...
    for(vl_index = 0; vl_index < 4; vl_index++)
    {
	 ...
	 if(err = ltr559_read_data_ps(ltr559_obj->client, &ltr559_obj->ps))
	  {
			APS_ERR("read data error\n");
	  }
	 
        vl_read_ps = ltr559_obj->ps;
...
exit_handle:
    ...
    /*
    if((err = ap3220_enable_ps(ltr559_obj->client, 0)))
    {
