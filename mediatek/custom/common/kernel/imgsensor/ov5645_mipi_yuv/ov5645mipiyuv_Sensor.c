...
bool OV5645MIPIYUV_read_otp(void)
{
	int i;
	kal_uint16 mid, lens_id;
	kal_uint32 address = 0x3d05;
	OV5645MIPI_write_cmos_sensor(0x3000, 0x00);
	OV5645MIPI_write_cmos_sensor(0x3004, 0xff);
	mDELAY(10);
	OV5645MIPI_write_cmos_sensor(0x3D21, 0x01);
	mDELAY(10);
#ifdef OV5645_OTP_DEBUG
	for(i=0; i<32; i++)
	{
            printk("[OV5645]mingji test otp data.res[0x%x]==0x%x\n",0x3D00+i,OV5645MIPIYUV_read_cmos_sensor(0x3D00+i));
	}
#endif
	for(i=0; i<3; i++)
	{
	    address = 0x3d05 + i*9;
	    printk("[OV5645]mingji test address == 0x%x\n",address);
	    mid = OV5645MIPIYUV_read_cmos_sensor(address);
	    lens_id = OV5645MIPIYUV_read_cmos_sensor(address+1);
	    if ((mid == 0x02)&&(lens_id == 0x36))  break;//truly ff modules.
	    if((mid == 0x06)&&(lens_id == 0x40)) break;
	}

	mDELAY(40);
	OV5645MIPI_write_cmos_sensor(0x3D21, 0x00);

	if (i<3)
	{
	    printk("[OV5645]mingji test. otp read success.");
	    return true;//darling af modules.
	}
	else
	{
	    printk("[OV5645]mingji test. otp read fail.");
	    return false;
	}
}
...
kal_uint32 OV5645MIPI_GetSensorID(kal_uint32 *sensorID)
{
	...
	for(i=0;i<3;i++)
	{
		...
		else if (OV5645MIPIYUV_read_otp()) //read mid, truly af modules mid is 0x02 and lens_id is 0x36.
		{
			OV5645MIPISENSORDB("[OV5645MIPI]This is truly ff modules.");
			*sensorID=OV5645MIPI_SENSOR_ID;
			break;
		}
		else
		{
		        OV5645MIPISENSORDB("[OV5645MIPI]This is other modules.");
			*sensorID =0xffffffff;
			return ERROR_SENSOR_CONNECT_FAIL;
		}
    #else
        ...
        {        
            ...
            OV5645MIPISENSORDB("[OV5645MIPI]res:%d, adcVol:%d mV\n", res, adcVol);
        	if(adcVol >= 500) //DOVDD
            {   
                *sensorID=OV5645MIPI_SENSOR_ID;
        	    OV5645MIPISENSORDB("[OV5645MIPI]This is truly modules.\n");
            }
            else
            {
                *sensorID = 0xFFFFFFFF;
                OV5645MIPISENSORDB("[OV5645MIPI]This isn't truly module.\n");                
            }
            break;
        }
    #endif
...
void OV5645MIPIInitialSetting(void)
{
	...
	OV5645MIPI_write_cmos_sensor(0x3406, 0x01);//	; awb manual, 	
	...
	OV5645MIPI_write_cmos_sensor(0x3031, 0x08);//	zhouliao:from 0x08	//
	OV5645MIPI_write_cmos_sensor(0x3611, 0x06);//   laimingji:from 0x06, low light noise 0xc6
	OV5645MIPI_write_cmos_sensor(0x3612, 0xab);//   laimingji:from 0xab, low light noise 0x2b
	...
	OV5645MIPI_write_cmos_sensor(0x3400, 0x05);// r
	OV5645MIPI_write_cmos_sensor(0x3401, 0x42); 
	OV5645MIPI_write_cmos_sensor(0x3402, 0x04);// g
	OV5645MIPI_write_cmos_sensor(0x3403, 0x00); 
	OV5645MIPI_write_cmos_sensor(0x3404, 0x07);// b
	OV5645MIPI_write_cmos_sensor(0x3405, 0x60); 
	...
	//1st for truly modules
     ...
     OV5645MIPI_write_cmos_sensor(0x5181, 0xf6);//
     ...
     OV5645MIPI_write_cmos_sensor(0x5186, 0xf );//
     OV5645MIPI_write_cmos_sensor(0x5187, 0x17);//
     OV5645MIPI_write_cmos_sensor(0x5188, 0xf );//
     OV5645MIPI_write_cmos_sensor(0x5189, 0x82);//
     OV5645MIPI_write_cmos_sensor(0x518a, 0x61);//
     OV5645MIPI_write_cmos_sensor(0x518b, 0xa7);//
     OV5645MIPI_write_cmos_sensor(0x518c, 0x8a);//
     OV5645MIPI_write_cmos_sensor(0x518d, 0x3b);//
     OV5645MIPI_write_cmos_sensor(0x518e, 0x36);//
     OV5645MIPI_write_cmos_sensor(0x518f, 0x53);//
     OV5645MIPI_write_cmos_sensor(0x5190, 0x49);//
     ...
     OV5645MIPI_write_cmos_sensor(0x5199, 0xed);//
     OV5645MIPI_write_cmos_sensor(0x519a, 0x4 );//
     OV5645MIPI_write_cmos_sensor(0x519b, 0x0 );//
     OV5645MIPI_write_cmos_sensor(0x519c, 0x7 );//
     OV5645MIPI_write_cmos_sensor(0x519d, 0xc7);//
     OV5645MIPI_write_cmos_sensor(0x519e, 0x38);//


        //CCM
	OV5645MIPI_write_cmos_sensor(0x5381, 0x27);//CCM
	OV5645MIPI_write_cmos_sensor(0x5382, 0x50);
	OV5645MIPI_write_cmos_sensor(0x5383, 0x11);
	OV5645MIPI_write_cmos_sensor(0x5384, 0x0a);
	OV5645MIPI_write_cmos_sensor(0x5385, 0x66);
	OV5645MIPI_write_cmos_sensor(0x5386, 0x71);
	OV5645MIPI_write_cmos_sensor(0x5387, 0x7c);
	OV5645MIPI_write_cmos_sensor(0x5388, 0x6b);
	OV5645MIPI_write_cmos_sensor(0x5389, 0x11);
	OV5645MIPI_write_cmos_sensor(0x538a, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x538b, 0x98);//

	OV5645MIPI_write_cmos_sensor(0x5308, 0x35);
	OV5645MIPI_write_cmos_sensor(0x5300, 0x08);//	; sharpen MT th1
	OV5645MIPI_write_cmos_sensor(0x5301, 0x48);//	; sharpen MT th2
	OV5645MIPI_write_cmos_sensor(0x5302, 0x28);//0x20);//	; sharpen MT off1
	OV5645MIPI_write_cmos_sensor(0x5303, 0x10);//0x00);//	; sharpen MT off2
	OV5645MIPI_write_cmos_sensor(0x5304, 0x08);//	; DNS th1
	OV5645MIPI_write_cmos_sensor(0x5305, 0x30);//	; DNS th2
	OV5645MIPI_write_cmos_sensor(0x5306, 0x10);//	; DNS off1 //0x08
	OV5645MIPI_write_cmos_sensor(0x5307, 0x20);//	; DNS off2 //0x16
	...
	OV5645MIPI_write_cmos_sensor(0x5484, 0x5a);//
	OV5645MIPI_write_cmos_sensor(0x5485, 0x68);//
	OV5645MIPI_write_cmos_sensor(0x5486, 0x79);//
	OV5645MIPI_write_cmos_sensor(0x5487, 0x83);//
	OV5645MIPI_write_cmos_sensor(0x5488, 0x90);//
	OV5645MIPI_write_cmos_sensor(0x5489, 0x9a);//
	OV5645MIPI_write_cmos_sensor(0x548a, 0xa4);//
	OV5645MIPI_write_cmos_sensor(0x548b, 0xb5);//
	OV5645MIPI_write_cmos_sensor(0x548c, 0xc0);//
	OV5645MIPI_write_cmos_sensor(0x548d, 0xce);//
...
/*
	OV5645MIPI_write_cmos_sensor(0x5480, 0x01);//	; bias on
	OV5645MIPI_write_cmos_sensor(0x5481, 0x0a);//	; Y yst 00
	OV5645MIPI_write_cmos_sensor(0x5482, 0x16);//
	OV5645MIPI_write_cmos_sensor(0x5483, 0x2e);//
	OV5645MIPI_write_cmos_sensor(0x5484, 0x57);//
	OV5645MIPI_write_cmos_sensor(0x5485, 0x69);//
	OV5645MIPI_write_cmos_sensor(0x5486, 0x7a);//
	OV5645MIPI_write_cmos_sensor(0x5487, 0x88);//
	OV5645MIPI_write_cmos_sensor(0x5488, 0x93);//
	OV5645MIPI_write_cmos_sensor(0x5489, 0x9f);//
	OV5645MIPI_write_cmos_sensor(0x548a, 0xa9);//
	OV5645MIPI_write_cmos_sensor(0x548b, 0xb7);//
	OV5645MIPI_write_cmos_sensor(0x548c, 0xc0);//
	OV5645MIPI_write_cmos_sensor(0x548d, 0xd1);//
	OV5645MIPI_write_cmos_sensor(0x548e, 0xdf);//
	OV5645MIPI_write_cmos_sensor(0x548f, 0xea);//
	OV5645MIPI_write_cmos_sensor(0x5490, 0x1d);//
*/
#endif
	...
	OV5645MIPI_write_cmos_sensor(0x5790, 0x01);//
#if 0
	OV5645MIPI_write_cmos_sensor(0x5580, 0x06);//UV
	...
	OV5645MIPI_write_cmos_sensor(0x558b, 0x88);//
#else
	OV5645MIPI_write_cmos_sensor(0x5580, 0x06);//UV
	OV5645MIPI_write_cmos_sensor(0x5588, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5583, 0x40);//
...
#if 1
	//From OV Kobe D65+tl84.
	OV5645MIPI_write_cmos_sensor(0x5800, 0x13);//lsc
	OV5645MIPI_write_cmos_sensor(0x5801, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5802, 0x0b);//
	OV5645MIPI_write_cmos_sensor(0x5803, 0x0b);//
	OV5645MIPI_write_cmos_sensor(0x5804, 0x0f);//
	OV5645MIPI_write_cmos_sensor(0x5805, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x5806, 0x0a);//
	...
	OV5645MIPI_write_cmos_sensor(0x580b, 0x0a);//
	OV5645MIPI_write_cmos_sensor(0x580c, 0x05);//
	OV5645MIPI_write_cmos_sensor(0x580d, 0x02);//
	OV5645MIPI_write_cmos_sensor(0x580e, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x580f, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5810, 0x02);//
	OV5645MIPI_write_cmos_sensor(0x5811, 0x05);//
	OV5645MIPI_write_cmos_sensor(0x5812, 0x05);//
	OV5645MIPI_write_cmos_sensor(0x5813, 0x02);//
	OV5645MIPI_write_cmos_sensor(0x5814, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5815, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5816, 0x02);//
	OV5645MIPI_write_cmos_sensor(0x5817, 0x05);//
	OV5645MIPI_write_cmos_sensor(0x5818, 0x0a);//
	OV5645MIPI_write_cmos_sensor(0x5819, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x581a, 0x04);//
	OV5645MIPI_write_cmos_sensor(0x581b, 0x04);//
	OV5645MIPI_write_cmos_sensor(0x581c, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x581d, 0x0a);//
	OV5645MIPI_write_cmos_sensor(0x581e, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x581f, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5820, 0x0b);//
	OV5645MIPI_write_cmos_sensor(0x5821, 0x0b);//
	OV5645MIPI_write_cmos_sensor(0x5822, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5823, 0x15);//
	OV5645MIPI_write_cmos_sensor(0x5824, 0x57);//
	OV5645MIPI_write_cmos_sensor(0x5825, 0x3a);//
	OV5645MIPI_write_cmos_sensor(0x5826, 0x1d);//
	OV5645MIPI_write_cmos_sensor(0x5827, 0x3a);//
	OV5645MIPI_write_cmos_sensor(0x5828, 0x66);//
	OV5645MIPI_write_cmos_sensor(0x5829, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x582a, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x582b, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x582c, 0x18);//
	OV5645MIPI_write_cmos_sensor(0x582d, 0x1c);//
	OV5645MIPI_write_cmos_sensor(0x582e, 0x1c);//
	OV5645MIPI_write_cmos_sensor(0x582f, 0x33);//
	OV5645MIPI_write_cmos_sensor(0x5830, 0x40);//
	OV5645MIPI_write_cmos_sensor(0x5831, 0x33);//
	OV5645MIPI_write_cmos_sensor(0x5832, 0x0c);//
	OV5645MIPI_write_cmos_sensor(0x5833, 0x1c);//
	OV5645MIPI_write_cmos_sensor(0x5834, 0x18);//
	OV5645MIPI_write_cmos_sensor(0x5835, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x5836, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x5837, 0x1b);//
	OV5645MIPI_write_cmos_sensor(0x5838, 0x55);//
	OV5645MIPI_write_cmos_sensor(0x5839, 0x3a);//
	OV5645MIPI_write_cmos_sensor(0x583a, 0x1d);//
	OV5645MIPI_write_cmos_sensor(0x583b, 0x29);//
	OV5645MIPI_write_cmos_sensor(0x583c, 0x65);//
	OV5645MIPI_write_cmos_sensor(0x583d, 0xcd);//
/*
	//From OV Kobe office.
	OV5645MIPI_write_cmos_sensor(0x5800, 0x1C);//lsc
	OV5645MIPI_write_cmos_sensor(0x5801, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x5802, 0x11);//
	OV5645MIPI_write_cmos_sensor(0x5803, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5804, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x5805, 0x1A);//
	OV5645MIPI_write_cmos_sensor(0x5806, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5807, 0x0A);//
	OV5645MIPI_write_cmos_sensor(0x5808, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x5809, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x580a, 0x09);//
	OV5645MIPI_write_cmos_sensor(0x580b, 0x0F);//
	OV5645MIPI_write_cmos_sensor(0x580c, 0x0A);//
	...
	OV5645MIPI_write_cmos_sensor(0x5811, 0x0A);//
	...
	OV5645MIPI_write_cmos_sensor(0x5818, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5819, 0x0A);//
	OV5645MIPI_write_cmos_sensor(0x581a, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x581b, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x581c, 0x0B);//
	OV5645MIPI_write_cmos_sensor(0x581d, 0x0E);//
	OV5645MIPI_write_cmos_sensor(0x581e, 0x1C);//
	OV5645MIPI_write_cmos_sensor(0x581f, 0x15);//
	OV5645MIPI_write_cmos_sensor(0x5820, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5821, 0x11);//
	OV5645MIPI_write_cmos_sensor(0x5822, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x5823, 0x20);//
	OV5645MIPI_write_cmos_sensor(0x5824, 0x41);//
	OV5645MIPI_write_cmos_sensor(0x5825, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x5826, 0x03);//
	OV5645MIPI_write_cmos_sensor(0x5827, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x5828, 0x41);//
	OV5645MIPI_write_cmos_sensor(0x5829, 0x43);//
	OV5645MIPI_write_cmos_sensor(0x582a, 0x21);//
	OV5645MIPI_write_cmos_sensor(0x582b, 0x41);//
	OV5645MIPI_write_cmos_sensor(0x582c, 0x21);//
	OV5645MIPI_write_cmos_sensor(0x582d, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x582e, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x582f, 0x41);//
	OV5645MIPI_write_cmos_sensor(0x5830, 0x81);//
	OV5645MIPI_write_cmos_sensor(0x5831, 0x41);//
	OV5645MIPI_write_cmos_sensor(0x5832, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5833, 0x43);//
	OV5645MIPI_write_cmos_sensor(0x5834, 0x21);//
	OV5645MIPI_write_cmos_sensor(0x5835, 0x21);//
	OV5645MIPI_write_cmos_sensor(0x5836, 0x21);//
	OV5645MIPI_write_cmos_sensor(0x5837, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x5838, 0x41);//
	OV5645MIPI_write_cmos_sensor(0x5839, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x583a, 0x45);//
	OV5645MIPI_write_cmos_sensor(0x583b, 0x23);//
	OV5645MIPI_write_cmos_sensor(0x583c, 0x45);//
	OV5645MIPI_write_cmos_sensor(0x583d, 0xAF);//
*/
#else
/*
	//From OV Kobe D65.
	OV5645MIPI_write_cmos_sensor(0x5800, 0x1e);//lsc
	OV5645MIPI_write_cmos_sensor(0x5801, 0x12);//
	OV5645MIPI_write_cmos_sensor(0x5802, 0x12);//
	OV5645MIPI_write_cmos_sensor(0x5803, 0x15);//
	OV5645MIPI_write_cmos_sensor(0x5804, 0x1e);//
	OV5645MIPI_write_cmos_sensor(0x5805, 0x27);//
	OV5645MIPI_write_cmos_sensor(0x5806, 0x0f);//
	OV5645MIPI_write_cmos_sensor(0x5807, 0x08);//
	OV5645MIPI_write_cmos_sensor(0x5808, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x5809, 0x09);//
	OV5645MIPI_write_cmos_sensor(0x580a, 0x0f);//
	OV5645MIPI_write_cmos_sensor(0x580b, 0x13);//
	OV5645MIPI_write_cmos_sensor(0x580c, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x580d, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x580e, 0x00);//
	OV5645MIPI_write_cmos_sensor(0x580f, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5810, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x5811, 0x0e);//
	OV5645MIPI_write_cmos_sensor(0x5812, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x5813, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5814, 0x00);//
	OV5645MIPI_write_cmos_sensor(0x5815, 0x01);//
	OV5645MIPI_write_cmos_sensor(0x5816, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x5817, 0x0e);//
	OV5645MIPI_write_cmos_sensor(0x5818, 0x0c);//
	OV5645MIPI_write_cmos_sensor(0x5819, 0x08);//
	OV5645MIPI_write_cmos_sensor(0x581a, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x581b, 0x09);//
	OV5645MIPI_write_cmos_sensor(0x581c, 0x0d);//
	OV5645MIPI_write_cmos_sensor(0x581d, 0x11);//
	OV5645MIPI_write_cmos_sensor(0x581e, 0x16);//
	OV5645MIPI_write_cmos_sensor(0x581f, 0x12);//
	OV5645MIPI_write_cmos_sensor(0x5820, 0x12);//
	OV5645MIPI_write_cmos_sensor(0x5821, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x5822, 0x17);//
	OV5645MIPI_write_cmos_sensor(0x5823, 0x21);//
	OV5645MIPI_write_cmos_sensor(0x5824, 0x46);//
	OV5645MIPI_write_cmos_sensor(0x5825, 0x0c);//
	OV5645MIPI_write_cmos_sensor(0x5826, 0x0e);//
	OV5645MIPI_write_cmos_sensor(0x5827, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x5828, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x5829, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x582a, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x582b, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x582c, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x582d, 0x0a);//
	OV5645MIPI_write_cmos_sensor(0x582e, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x582f, 0x44);//
	OV5645MIPI_write_cmos_sensor(0x5830, 0x40);//
	OV5645MIPI_write_cmos_sensor(0x5831, 0x24);//
	OV5645MIPI_write_cmos_sensor(0x5832, 0x08);//
	OV5645MIPI_write_cmos_sensor(0x5833, 0x2A);//
	OV5645MIPI_write_cmos_sensor(0x5834, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x5835, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x5836, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x5837, 0x2a);//
	OV5645MIPI_write_cmos_sensor(0x5838, 0x46);//
	OV5645MIPI_write_cmos_sensor(0x5839, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x583a, 0x2c);//
	OV5645MIPI_write_cmos_sensor(0x583b, 0x2a);//
	OV5645MIPI_write_cmos_sensor(0x583c, 0x28);//
	OV5645MIPI_write_cmos_sensor(0x583d, 0xcc);//
*/
	//From OV Kobe CWF.
	OV5645MIPI_write_cmos_sensor(0x5800, 0x19);//lsc
	OV5645MIPI_write_cmos_sensor(0x5801, 0x0E);//
	OV5645MIPI_write_cmos_sensor(0x5802, 0x0F);//
	OV5645MIPI_write_cmos_sensor(0x5803, 0x12);//
	OV5645MIPI_write_cmos_sensor(0x5804, 0x1A);//
	OV5645MIPI_write_cmos_sensor(0x5805, 0x2C);//
	OV5645MIPI_write_cmos_sensor(0x5806, 0x0E);//
	OV5645MIPI_write_cmos_sensor(0x5807, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x5808, 0x05);//
	OV5645MIPI_write_cmos_sensor(0x5809, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x580a, 0x0B);//
	OV5645MIPI_write_cmos_sensor(0x580b, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x580c, 0x0A);//
	OV5645MIPI_write_cmos_sensor(0x580d, 0x02);//
	OV5645MIPI_write_cmos_sensor(0x580e, 0x00);//
	OV5645MIPI_write_cmos_sensor(0x580f, 0x00);//
	OV5645MIPI_write_cmos_sensor(0x5810, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x5811, 0x0F);//
	OV5645MIPI_write_cmos_sensor(0x5812, 0x0A);//
	...
	OV5645MIPI_write_cmos_sensor(0x5816, 0x06);//
	OV5645MIPI_write_cmos_sensor(0x5817, 0x0E);//
	OV5645MIPI_write_cmos_sensor(0x5818, 0x0E);//
	OV5645MIPI_write_cmos_sensor(0x5819, 0x08);//
	OV5645MIPI_write_cmos_sensor(0x581a, 0x07);//
	OV5645MIPI_write_cmos_sensor(0x581b, 0x08);//
	OV5645MIPI_write_cmos_sensor(0x581c, 0x0E);//
	OV5645MIPI_write_cmos_sensor(0x581d, 0x14);//
	OV5645MIPI_write_cmos_sensor(0x581e, 0x16);//
	OV5645MIPI_write_cmos_sensor(0x581f, 0x11);//
	OV5645MIPI_write_cmos_sensor(0x5820, 0x10);//
	OV5645MIPI_write_cmos_sensor(0x5821, 0x13);//
	OV5645MIPI_write_cmos_sensor(0x5822, 0x16);//
	OV5645MIPI_write_cmos_sensor(0x5823, 0x24);//
	OV5645MIPI_write_cmos_sensor(0x5824, 0x64);//
	OV5645MIPI_write_cmos_sensor(0x5825, 0x46);//
	OV5645MIPI_write_cmos_sensor(0x5826, 0x26);//
	OV5645MIPI_write_cmos_sensor(0x5827, 0x46);//
	OV5645MIPI_write_cmos_sensor(0x5828, 0x66);//
	OV5645MIPI_write_cmos_sensor(0x5829, 0x44);//
	OV5645MIPI_write_cmos_sensor(0x582a, 0x64);//
	OV5645MIPI_write_cmos_sensor(0x582b, 0x62);//
	OV5645MIPI_write_cmos_sensor(0x582c, 0x42);//
	OV5645MIPI_write_cmos_sensor(0x582d, 0x24);//
	OV5645MIPI_write_cmos_sensor(0x582e, 0x44);//
	OV5645MIPI_write_cmos_sensor(0x582f, 0x82);//
	OV5645MIPI_write_cmos_sensor(0x5830, 0xA0);//
	OV5645MIPI_write_cmos_sensor(0x5831, 0x40);//
	OV5645MIPI_write_cmos_sensor(0x5832, 0x04);//
	OV5645MIPI_write_cmos_sensor(0x5833, 0x64);//
	OV5645MIPI_write_cmos_sensor(0x5834, 0x42);//
	OV5645MIPI_write_cmos_sensor(0x5835, 0x22);//
	OV5645MIPI_write_cmos_sensor(0x5836, 0x42);//
	OV5645MIPI_write_cmos_sensor(0x5837, 0x46);//
	OV5645MIPI_write_cmos_sensor(0x5838, 0x86);//
	OV5645MIPI_write_cmos_sensor(0x5839, 0x66);//
	OV5645MIPI_write_cmos_sensor(0x583a, 0x68);//
	OV5645MIPI_write_cmos_sensor(0x583b, 0x66);//
	OV5645MIPI_write_cmos_sensor(0x583c, 0x88);//
	OV5645MIPI_write_cmos_sensor(0x583d, 0x8E);//
#endif
#if 0
...
//#else
	OV5645MIPI_write_cmos_sensor(0x5688, 0x11);
	OV5645MIPI_write_cmos_sensor(0x5689, 0x11);
	OV5645MIPI_write_cmos_sensor(0x568a, 0x41);
	OV5645MIPI_write_cmos_sensor(0x568b, 0x14);
	OV5645MIPI_write_cmos_sensor(0x568c, 0x42);
	OV5645MIPI_write_cmos_sensor(0x568d, 0x24);
	OV5645MIPI_write_cmos_sensor(0x568e, 0x21);
	OV5645MIPI_write_cmos_sensor(0x568f, 0x12);
#endif
	...
	OV5645MIPI_write_cmos_sensor(0x3a1b, 0x29);//	; AEC out H
	OV5645MIPI_write_cmos_sensor(0x3a1e, 0x23);//	; AEC out L
	OV5645MIPI_write_cmos_sensor(0x3a11, 0x53);//	; control zone H
...
static void OV5645MIPI_FlashTriggerCheck(unsigned int *pFeatureReturnPara32)
{
	...
	OV5645MIPISENSORDB("[OV5645MIPI] OV5645MIPI_FlashTriggerCheck: %x\n", NormBr);
